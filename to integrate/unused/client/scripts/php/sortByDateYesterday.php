<?php
	...
	$select_pictures_by_date_yesterday = $connexion->prepare("SELECT photo_url, photo_title, photo_owner_id, photo_id FROM Photos WHERE photo_date_upload_tri > :difference ORDER BY photo_date_upload_tri DESC");
	//On lie notre variable
	$select_pictures_by_date_yesterday->bindValue('difference', $difference, PDO::PARAM_INT);
	//On exécute
	$select_pictures_by_date_yesterday->execute();
	
	//Tant qu'on a une ligne à afficher
	while($result_by_date_yesterday = $select_pictures_by_date_yesterday->fetch()){
		
		//On récupère les infos utiles
		$url_photo = $result_by_date_yesterday['photo_url'];
		$title_photo = $result_by_date_yesterday['photo_title'];
		$id_autor_photo = $result_by_date_yesterday['photo_owner_id'];
		$id_image = $result_by_date_yesterday['photo_id'];
		//$sent_ago = $current_date - $result_by_date_yesterday['photo_date_upload_tri'];
	...
	//Clore la requête préparée
	$select_pictures_by_date_yesterday->closeCursor();
	$select_pictures_by_date_yesterday = NULL;
?>