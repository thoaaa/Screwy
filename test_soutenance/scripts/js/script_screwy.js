...
function webSocket() {
	...
	connection.onopen = function () {
		console.log("Ouverture connexion");
		globalConnexion = this;
...
function sendInstruction(t) {
	...
	if (type == "message") {
		...
		var salon = document.getElementById("id_salon_hidden").value;
		instruction.type = type;
		instruction.pseudo = pseudo;
		instruction.key = key;
		instruction.salon = salon;
	}
	var msg = JSON.stringify(instruction);
	console.log("send : " + msg );
	globalConnexion.send(msg);
	return false;
}

function sendMsg() {
	webSocket.sendInstruction('message');
	return false;
}
